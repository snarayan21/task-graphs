...
def main():
    ...
    plt.rc('axes', labelsize=MED_BIG) # fontsize of the x and y labels
    ...
    fig, axs = plt.subplots(1, 3, gridspec_kw={'width_ratios':[1,1,0.08], 'wspace':0.6})
    #fig.tight_layout(pad=0.5)
    plt.subplots_adjust(left=0.1, right=0.9,bottom=0.15,top=0.9, wspace=0.3)
    g1 = sns.heatmap(flow_rewards_plot, linewidth=0.5, cmap='seismic_r', center=1, xticklabels=[8, 12, 16, 20], yticklabels=[1.0, 0.75, 0.50, 0.25], ax=axs[0], vmin=0, vmax=10, cbar_ax=axs[2], cbar_kws={"label":"Normalized Reward","pad":0,"extend":'max',"ticks":[0,1,2,4,6,8,10]}, square=True)
    g2 = sns.heatmap(greedy_rewards_plot, linewidth=0.5, cmap='seismic_r', cbar=False, center=1, xticklabels=[8, 12, 16, 20], yticklabels=[1.0, 0.75, 0.50, 0.25], ax=axs[1], vmin=0, vmax=10, square=True)# cbar_kws={"label":"%"}
    ...
    g1.set_title('Flow NLP Reward (Norm)')
    g2.set_title('Greedy Reward (Norm)')
