            ...
            if self.constraint_type == 'qp':
                ...
                if(l == self.pred_time - 1):
                    ...
                    if(self.constraint_buffer == 'soft'):
                        ...
                        col[-1] = 3e-2*(1 + abs(P.max()))*(curr_iter + 1)
                        ...
                        q = np.copy(np.atleast_1d(np.squeeze(q_u)))
                        #print("q before:", q)
                        q = np.append(q, [3e-2*(1 + abs(q.max()))*(curr_iter + 1)])
                        print("k P coefficient is: ", 3e-2*(1 + abs(P.max()))*(curr_iter + 1))
                        print("k q coefficient is: ", 3e-2*(1 + abs(q.max()))*(curr_iter + 1))
                        #print("q after:", q)
