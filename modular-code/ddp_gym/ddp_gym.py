...
class DDP:
    ...
    def backward(self, x_seq, u_seq):
        ...
        for l in range(self.pred_time - 1, -1, -1): # (num_tasks-2, num_tasks-3, ..., 0)
            ...
            print("l is:", l)
            print("l_ind is:", l_ind)
            print("incoming u seq is:", incoming_u_seq[l])
            print("x is:", x)
            print("additional_x is:", additional_x)
            print(f_x)
            print(np.atleast_1d(x))
            print(np.atleast_1d(incoming_u_seq[l]))
            print(np.atleast_1d(additional_x))
            print(f_x(np.atleast_1d(x), np.atleast_1d(incoming_u_seq[l]), np.atleast_1d(additional_x), l_ind))
            print(f_x_t)
            #breakpoint()
